...
def test_tui_renders_temporary_permission_component_and_artifact_messages(tmp_path: Path) -> None:
    app = NoteDeskTUI(
        workspace=tmp_path,
        artifact_root=tmp_path / "artifacts",
    )
...
def test_tui_permission_prompt_shows_requested_operation(tmp_path: Path) -> None:
    app = NoteDeskTUI(tmp_path, tmp_path / "artifacts")
    ...
    assert app._permission_divider.char == "─"
    assert app._permission_divider.style == "class:permission.divider"
